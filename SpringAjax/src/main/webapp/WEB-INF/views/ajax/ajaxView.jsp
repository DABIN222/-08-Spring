...
<style>
	#resultView{
		padding:20px; border:1px solid gray; background-color:lightblue; height:200px;
		margin:20px;
	}
	...
	.pList>li{
		float:left; height:40px; line-height:40px; border-bottom:1px solid gray;
		width:25%
	}
...
<script>
	$(function(){
		...
		$("#ajaxString").click(function(){
			...
			var params = "num=100&name=홍길동&addr=서울시 강남구"
			$.ajax({
				url : url,		// 서버의 매핑주소
				type : 'GET',	// 전송 방식
				data : params,	// 서버로 보낼 데이터
				success : function(result){		// 서버에서 성공적으로 응답을 받으면 실행할 함수, 데이터는 result 매개변수에 담긴다.
					// 서버에서 받은 값(데이터)을 뷰페이지에 표시
					$("#resultView").html(result);
				},
				error : function(error){		// 에러 발생시 콘솔에 찍힘
		...
		$("#ajaxList").click(function(){
			...
			$.ajax({
				...
				success : function(result){
					...
					var $result= $(result);		// $ 선택한 변수이다 라는 뜻 $result
		...
		$("#ajaxJson").click(function(){
			...
			var data = {no:5555, name:'이순신', tel:'010-1111-1111'};
...
<button id="ajaxJson">ajax(Jason)</button>
<hr/>
